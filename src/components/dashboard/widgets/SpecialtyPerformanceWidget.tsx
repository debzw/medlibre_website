    ...
    return (
        <DashboardWidget colSpan={2} title="Desempenho por Especialidade" icon={MedLibreLogo} loading={loading}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10 pt-4">
                {data.length > 0 ? (
                    data.map((item) => (
                        <Link href={`/performance/${encodeURIComponent(item.name)}`} className="w-full">
                            <div className="group/item relative flex flex-col gap-3 transition-all duration-500 hover:bg-muted/10 p-2 rounded-xl -mx-2 cursor-pointer">
