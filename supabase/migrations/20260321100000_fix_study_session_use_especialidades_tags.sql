-- ============================================================
-- Migration: Fix study session RPCs to filter by especialidades_tags
-- ============================================================
-- Problem: get_study_session_questions and get_study_session_questions_v2
-- filter specialty via `output_especialidade = p_especialidade` (scalar),
-- but the filter UI (useSmartFilters → get_question_metadata_summary) produces
-- specialty values by unnesting `especialidades_tags` (TEXT[]).
-- Selecting a specialty therefore showed correct counts but fetched wrong/empty sessions.
--
-- Fix: replace every `output_especialidade = p_especialidade` with
--      `p_especialidade = ANY(especialidades_tags)` in both functions,
-- and rewrite the cold-start sampling to iterate over distinct tags from
-- `unnest(especialidades_tags)` instead of distinct `output_especialidade`.
-- ============================================================


-- ── 1. get_study_session_questions ───────────────────────────────────────────
...
BEGIN
    ...
    -- 5. LECTOR: branch via decs_hierarquia_encontrada
...
-- ── 2. get_study_session_questions_v2 ────────────────────────────────────────
