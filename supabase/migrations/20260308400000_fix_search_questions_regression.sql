-- ============================================================
-- Migration: fix_search_questions_regression
-- Description: Fixes critical bugs in hierarchical search:
--  1. Fixes join column (qd.id -> qd.decs_id)
--  2. Fixes array concat with NULL result
--  3. Fixes illegal use of 'score' alias in WHERE clause
--  4. Standardizes logic using LATERAL for clean, reusable scores
-- ============================================================
...
BEGIN
  ...
  IF v_decs_id IS NOT NULL THEN
    v_desc_decs_ids := ARRAY[v_decs_id];
    ...
    IF v_desc_decs_ids IS NOT NULL AND array_length(v_desc_decs_ids, 1) > 0 THEN
      ...
      WITH best_match AS (
        -- CRITICAL FIX: Join on decs_id, not id
