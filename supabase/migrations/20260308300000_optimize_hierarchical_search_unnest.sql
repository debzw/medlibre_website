-- ============================================================
-- Migration: optimize_hierarchical_search_unnest
-- Description: Replaces ANY(array) with unnest joins and an explicit 
-- early GROUP BY to avoid expensive Sequential Scans or repeated JOINs.
-- Merges Attempt 1 & 2 into a single unified step.
-- ============================================================
...
BEGIN
  ...
  -- ── Structured path: direct + hierarchical combined ───────
  IF v_decs_id IS NOT NULL THEN

    v_desc_decs_ids := ARRAY[v_decs_id]; -- ensure parent is included
    ...
    IF v_desc_decs_ids IS NOT NULL AND array_length(v_desc_decs_ids, 1) > 0 THEN
      ...
      ranked AS (
        SELECT 
          ...
          bm.tag_multiplier * GREATEST(0.3, 1.0 - (EXTRACT(YEAR FROM NOW())::INTEGER - q.ano) * 0.02) AS score
        FROM best_match bm
        ...
        WHERE
          ...
          -- Ensure pagination logic correctly filters before the limit clause
          AND (
            p_last_score IS NULL OR p_last_id IS NULL
            OR (bm.tag_multiplier * GREATEST(0.3, 1.0 - (EXTRACT(YEAR FROM NOW())::INTEGER - q.ano) * 0.02), q.id::text)
        ...
        OR (
          CASE
            ...
            ELSE
              ts_rank(to_tsvector('portuguese', q.enunciado), v_tsquery)
              * GREATEST(0.3, 1.0 - (EXTRACT(YEAR FROM NOW())::INTEGER - q.ano) * 0.02)
          END, q.id::text) < (p_last_score, p_last_id::text)
      )
  )
  SELECT json_build_object(
    'results',        COALESCE(json_agg(row_to_json(r.*) ORDER BY r.score DESC, r.id DESC), '[]'::json),
