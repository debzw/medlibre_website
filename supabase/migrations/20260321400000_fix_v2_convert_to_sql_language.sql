-- ============================================================
-- Fix: get_study_session_questions_v2 — converter para LANGUAGE sql
--
-- Problema raiz: funções PL/pgSQL com RETURNS TABLE expõem os nomes
-- das colunas como variáveis OUT em escopo por toda a função. Isso causou:
--   1. "column reference 'id' is ambiguous"       (migration 20260321300000)
--   2. "structure of query does not match function result type" (erro atual)
--
-- Fix definitivo: LANGUAGE sql. Em SQL functions os parâmetros OUT do
-- RETURNS TABLE NÃO ficam em escopo no corpo SQL, eliminando qualquer
-- colisão de nome/tipo. O corpo SQL usa nomes de coluna normais do SQL
-- resolvidos pelo escopo de CTE/tabela.
--
-- A bifurcação cold_start / standard é expressa via UNION ALL com
-- cláusulas WHERE mutuamente exclusivas (cnt < 50 / >= 50) que
-- substituem o IF/THEN exclusivo do PL/pgSQL.
-- ============================================================
...
-- Seleciona 2 questões por especialidade (máxima cobertura de áreas).
-- cs_tags e cs_questions só retornam linhas quando cnt < 50.
...
-- Os CTEs answered_ids e srs_due têm a condição cnt >= 50
-- para não executar trabalho pesado para usuários cold start.
