-- ============================================================
-- Fix: get_study_session_questions_v2 — add missing columns
--
-- The RETURNS TABLE definition was missing several columns that
-- exist in the questions table, most notably texto_base.
-- This caused texto_base to be undefined on the frontend for
-- all logged-in users (who always hit this RPC).
-- ============================================================

DROP FUNCTION IF EXISTS public.get_study_session_questions_v2(UUID,INTEGER,BOOLEAN,TEXT,INTEGER,TEXT,TEXT,TEXT);

CREATE FUNCTION public.get_study_session_questions_v2(
    p_user_id        UUID,
...
RETURNS TABLE(
    ...
    output_tema                 TEXT,
    output_subtema              TEXT,
    output_taxa_certeza         NUMERIC,
    processado                  INTEGER,
    tem_anomalia                INTEGER,
    log_anomalia                TEXT,
    created_at                  TIMESTAMPTZ,
    id_integracao               TIMESTAMPTZ,
...
AS $$
WITH
-- Conta histórico para decidir caminho (< 50 = cold start, >= 50 = standard)
history_count AS MATERIALIZED (
    SELECT COUNT(*)::INTEGER AS cnt
    FROM public.user_question_history
    WHERE user_id = p_user_id
),

-- ── COLD START path (cnt < 50) ──────────────────────────────────────────────
cs_tags AS MATERIALIZED (
    ...
    WHERE (p_banca         IS NULL OR banca              = p_banca)
      ...
      AND (SELECT cnt FROM history_count) < 50
),
cs_questions AS (
    SELECT q.*
    FROM cs_tags esp
...
cold_start AS (
    SELECT
        q.id, q.banca, q.ano, q.enunciado, q.texto_base, q.texto_base_processado,
        q.imagem_url, q.opcoes, q.resposta_correta,
        q.status_imagem, q.referencia_imagem,
        q.alternativa_a, q.alternativa_b, q.alternativa_c, q.alternativa_d, q.alternativa_e,
        q.imagem_alt_a, q.imagem_alt_b, q.imagem_alt_c, q.imagem_alt_d, q.imagem_alt_e,
        q.imagem_nova, q.especialidade, q.especialidades_tags,
        q.numero, q.tipo, q.exam_type,
        q.output_gabarito, q.output_explicacao, q.output_grande_area,
        q.output_especialidade, q.output_tema, q.output_subtema,
        q.output_taxa_certeza, q.processado, q.tem_anomalia, q.log_anomalia,
        q.created_at, q.id_integracao, q.decs_hierarquia_encontrada,
...
srs_due AS MATERIALIZED (
    SELECT q.*, 1 AS priority, r.retrievability::FLOAT8 AS weight
    FROM public.questions q
...
weak_theme_questions AS MATERIALIZED (
    SELECT q.*, 2 AS priority, (1.0 - wt.mastery_score)::FLOAT8 AS weight
    FROM public.questions q
...
discovery_questions AS MATERIALIZED (
    SELECT q.*, 3 AS priority, RANDOM()::FLOAT8 AS weight
...
general_new AS (
    SELECT q.*, 4 AS priority, RANDOM()::FLOAT8 AS weight
...
standard AS (
    SELECT
        id, banca, ano, enunciado, texto_base, texto_base_processado,
        imagem_url, opcoes, resposta_correta,
        status_imagem, referencia_imagem,
        alternativa_a, alternativa_b, alternativa_c, alternativa_d, alternativa_e,
        imagem_alt_a, imagem_alt_b, imagem_alt_c, imagem_alt_d, imagem_alt_e,
        imagem_nova, especialidade, especialidades_tags,
        numero, tipo, exam_type,
        output_gabarito, output_explicacao, output_grande_area,
        output_especialidade, output_tema, output_subtema,
        output_taxa_certeza, processado, tem_anomalia, log_anomalia,
        created_at, id_integracao, decs_hierarquia_encontrada,
        CASE priority
            WHEN 1 THEN 'srs'
...
-- cold_start e standard são mutuamente exclusivos (cnt < 50 vs >= 50)
