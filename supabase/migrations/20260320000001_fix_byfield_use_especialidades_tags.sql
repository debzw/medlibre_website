-- ============================================================
-- Migration: byField shows ALL specialties in the area,
-- including unstudied ones (total=0).
-- Uses DISTINCT unnest(especialidades_tags, question_id) then
-- a direct LEFT JOIN with user history — no correlated subquery.
-- ============================================================
...
BEGIN
  ...
  -- byField: per-speciality (especialidades_tags) stats for the given grande área.
  -- Strategy: unnest tags together with question_id, then LEFT JOIN user history
  -- directly on question_id — clean, no correlated subquery.
  SELECT COALESCE(
    json_object_agg(
      sub.tag,
      json_build_object(
  ...
  FROM (
    ...
    FROM (
      -- All distinct (tag, question_id) pairs for questions in this grande área
      SELECT DISTINCT
